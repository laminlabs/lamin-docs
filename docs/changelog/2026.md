...
- 🚸 Simplify connecting to an instance [PR](https://github.com/laminlabs/laminr/pull/258) [@lazappi](https://github.com/lazappi)
  ...
  Many of the checks when connecting to a `lamindb` instance have been removed.
  ...
  Due to these changes, `use_temporary_instance()` and the `get_current_lamin_*()` utils now operate in the main session instead of a separate session via **{callr}**.
