...
| **SQLRecord** field | `_branch_code` | `branch_id` |
...
| What               | Before                                      | After                                            |
| ------------------ | ------------------------------------------- | ------------------------------------------------ |
| **Artifact** field | `_is_saved_to_storage_location` (in `_aux`) | `_save_completed` (`BooleanField`, then removed) |
...
Changes in unused API components:

| What                | Before      | After                                |
| ------------------- | ----------- | ------------------------------------ |
| **Run** field       | `_logfile`  | removed                              |
| **RootBlock** model | `RootBlock` | `Block`                              |
| **Block** field     | `context`   | `key` (`CharField(max_length=1024)`) |
...
- 🏗️ Introduce `Feature._dtype_str` to hold a serialization that links object types (`Record` + `ULabel`) based on their `uid` instead of their `name` and type hierarchy [PR](https://github.com/laminlabs/lamindb/pull/3418) [@falexwolf](https://github.com/falexwolf)
