...
This release adds support for spaces and branches, configurable artifact keys, and manual path tracking.

### New functionality

- **Space & Branch support**: New `space_uid` and `branch_uid` top-level config options to scope transforms, runs, and artifacts into a specific space and branch ([#132](https://github.com/laminlabs/nf-lamin/pull/132)).
  ```groovy
  lamin {
...
- **Configurable artifact keys**: New `key` option on `output_artifacts`, `input_artifacts`, and individual rules to control how artifact keys are derived. Accepts a string template, a closure, or a `[relativize: ...]` map shorthand ([#133](https://github.com/laminlabs/nf-lamin/pull/133), [#135](https://github.com/laminlabs/nf-lamin/pull/135)).
  ```groovy
  lamin {
    output_artifacts {
...
- **Manual path tracking**: New `include_paths` setting on `input_artifacts`, `output_artifacts`, and individual rules to explicitly specify files that should be tracked — useful for files consumed in Groovy that are invisible to the task listener ([#139](https://github.com/laminlabs/nf-lamin/pull/139)).
  ```groovy
  lamin {
    input_artifacts {
...
- **Name-based references**: UID fields (`project_uids`, `ulabel_uids`, `space_uid`, `branch_uid`) now accept prefixed names (`?name`, `!name`, `+name`) for convenience and cross-environment testing ([#132](https://github.com/laminlabs/nf-lamin/pull/132)). Note that this is experimental and may be subject to change in future releases.

### Bug fixes

- Fixed key relativization to work uniformly across `file://`, `s3://`, `gs://`, and `az://` paths using `URI.relativize()` ([#135](https://github.com/laminlabs/nf-lamin/pull/135)).
- Fixed assets directory detection for users with custom `NXF_HOME` or `NXF_ASSETS` by using `nextflow.Const.DEFAULT_ROOT` instead of a hardcoded path ([#134](https://github.com/laminlabs/nf-lamin/pull/134)).

### Breaking changes

- `project_uids` inside `run {}`, `transform {}`, and artifact rules is removed. Use the top-level field only ([#132](https://github.com/laminlabs/nf-lamin/pull/132)).

### Internal

- ♻️ Restructured instance API functions and added tests ([#129](https://github.com/laminlabs/nf-lamin/pull/129)).

### Documentation

- 📝 Refactored documentation ([#136](https://github.com/laminlabs/nf-lamin/pull/136)).
