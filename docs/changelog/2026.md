...
- ✨ Add `lamin settings cache-dir reset` CLI command for resetting the cache directory to the default path [PR](https://github.com/laminlabs/lamindb/pull/3517) [@Koncopd](https://github.com/Koncopd)
- ✨ Add `S3QueryPath.is_bucket_versioned()` to check if the bucket in an s3 path is versioned [PR](https://github.com/laminlabs/lamindb-setup/pull/1279) [@Koncopd](https://github.com/Koncopd)
...
- 🐛 Fix `RuntimeError: dictionary changed size during iteration` in `reset_django_module_variables` on `lamindb_setup.init(...)` [PR](https://github.com/laminlabs/lamindb-setup/pull/1280) [@Koncopd](https://github.com/Koncopd)
