...
:::{dropdown} ⚠️ No longer deduplicate during the registration of already existing artifacts in storage, e.g., when an S3 path is passed to `Artifact()` [PR](https://github.com/laminlabs/lamindb/pull/3700) [@Koncopd](https://github.com/Koncopd)

Given a file or folder is already ingested into a storage location, `Artifact(f"s3://{storage_loc}/...")` will no longer attempt deduplicating (`skip_hash_lookup = True`).
...
- ✨ Move artifact in storage on key & suffix changes [PR](https://github.com/laminlabs/lamindb/pull/3673) [PR](https://github.com/laminlabs/lamindb/pull/3665) [@Koncopd](https://github.com/Koncopd)
