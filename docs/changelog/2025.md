...
- a UI for LaminDB's lakehouse, lineage, feature store, ontologies, LIMS, and ELN capabilities
- easy management of many LaminDB instances including hosting, access management, audit logs, and more

Through the open core you have full admin/developer access to your your data at zero lock-in danger. Instead of depending on a rate-limited REST, you build directly with Postgres & diverse open storage formats on AWS S3. This is possible because LaminHub connects databases and storage locations in their host data centers where you run your compute loads. Most functionality is available for AWS data centers, with support for other cloud providers and fully on-prem solutions also possible.
