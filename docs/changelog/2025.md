...
Many data structures allow storing metadata in dictionaries stored in specific slots like `.attrs` for Pandas DataFrames & SpatialData objects or `.uns` for AnnData and MuData objects.
It is now possible to validate such metadata.
...
Sometimes additional metadata is not necessarily stored in one of these slots but the dataset should still be annotated with specific metadata.
LaminDB now supports validating linked features:
