...
- 🏗️ Integrate the `Param` into the `Feature` registry [PR](https://github.com/laminlabs/lamindb/pull/2763) [@falexwolf](https://github.com/falexwolf) -- the change is backward compat, on the SQL level, records are transferred from the `lamindb_param` table to the `lamindb_feature` table during migrations
...
- ✨ Introduce a flexible `Record` registry to manage any kind of entity without migrations [PR](https://github.com/laminlabs/lamindb/pull/2782) [@falexwolf](https://github.com/falexwolf)
- 🏗️ Additional database level changes [PR](https://github.com/laminlabs/lamindb/pull/2794)
  ...
  - hash is populated for all `FeatureValue` records so that we have an easy way to universally identify a unique feature value
