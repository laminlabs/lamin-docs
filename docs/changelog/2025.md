...
LaminHub is a data collaboration hub built on LaminDB, similar to how GitHub is built on git. It provides two main features:

- a GUI for LaminDB's lakehouse, lineage, feature store, ontologies, LIMS, and ELN capabilities
- easy management of many LaminDB instances including hosting, access management, audit logs, and more

Through the open core users have full admin/developer access to their data at zero lock-in danger.

Instead of depending on a rate-limited REST, you build directly with Postgres & diverse open storage formats on AWS S3. This is possible because LaminHub connects databases and storage locations in their host data centers where you run your compute loads. Most functionality is available for AWS data centers, with support for other cloud providers and fully on-prem solutions also possible.
...
- 🚸 Create a tooltip to help orient the user in the "Create record" dialogue [PR](https://github.com/laminlabs/laminhub-public/pull/87) [@falexwolf](https://github.com/chaichontat)
- 💄 Enhance color for `Record` icon [PR](https://github.com/laminlabs/laminhub-public/pull/89) [@falexwolf](https://github.com/falexwolf)
- ✨ Auto-select unique transforms for `SheetExecute` [PR](https://github.com/laminlabs/laminhub-public/pull/88) [@falexwolf](https://github.com/chaichontat)
