...
🪜 For older changes, see: [2024](https://docs.lamin.ai/changelog/2024) · [2023](https://docs.lamin.ai/changelog/2023) · [2022](https://docs.lamin.ai/changelog/2022)
...
## 2025-01-16 {small}`db 1.0.0`

This release makes the API consistent, integrates `lnschema_core` & `ourprojects` into the `lamindb` package, and introduces a breadth of database migrations to enable future features without disruption.
...
2. After that, you will be prompted to call `lamin migrate deploy` to apply database migration

New features:

- ✨ Allow to `Artifact` to be based on a http URL [PR1](https://github.com/laminlabs/lamindb/pull/2291) [PR2](https://github.com/laminlabs/lamindb/pull/2330) [PR3](https://github.com/laminlabs/lamindb-setup/pull/925) [@Koncopd](https://github.com/Koncopd)
...
- ✨ Add relationship to `Project` to everything except `Run`, `Storage` & `User` so that you can easily filter for the entities relevant to your project [PR](https://github.com/laminlabs/lamindb/pull/2353) [@falexwolf](https://github.com/falexwolf)
...
- 🚸 Simplify the `ln.track()` experience [PR](https://github.com/laminlabs/lamindb/pull/2308) [@falexwolf](https://github.com/falexwolf)
  1. letting you omit the `uid` argument
  2. enabling you to organize transforms in folders
  3. automating the versioning process (requirement for 1.)
  4. enabling to save scripts and notebooks without running them (corollary of 1.)
  5. avoiding the interactive prompt in a notebook and the throwing of an error in a script (corollary of 1.)
  6. no longer requiring titles in notebooks
...
- 🐛 Fix renaming feature sets [PR](https://github.com/laminlabs/lamindb/pull/2288) [@sunnyosun](https://github.com/sunnyosun)
...
- rename `Transform.name` to `Transform.description` because it's analogous to `Artifact.description`
  ...
  - data migrations:
    - there already was a legacy `description` field that was never exposed on the constructor; to be safe, we concatenated potential data in it on the new description field [Claude chat](https://claude.ai/share/29b5c2a5-4922-417f-8d99-2563de50d535)
